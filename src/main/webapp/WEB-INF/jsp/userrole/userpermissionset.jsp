					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								...
								<form action="/tmms/userrole/permissionset" method="post" id="Paramform"
									class="form-horizontal">
									<div class=" col-xs-12 table-header">角色权限设置<a href="/tmms/userrole/userroles" style="float:right;color:#fff;">返回角色列表</a></div>
                                    <table id="dynamic-table" style="width:20%;">
                                    <th colspan=2>选择权限</th>
                                    <input type="hidden" name="roleId" id="roleId" value="${roleId }"/>
                                    <c:forEach items="${userPermissions}" var="userPermission" varStatus="status">
                                      <tr>
                                        <c:choose>
                                          <c:when test="${userPermission.extend1==1 }">
                                              <td><input type="checkbox" name="permissionId" id="permissionId" value="${userPermission.id }" checked="true"/></td>
                                          </c:when>
                                          <c:otherwise>
                                              <td><input type="checkbox" name="permissionId" id="permissionId" value="${userPermission.id }" /></td>
                                          </c:otherwise>
                                        </c:choose>
