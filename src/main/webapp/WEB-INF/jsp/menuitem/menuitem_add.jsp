					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								...
								<sf:form action="menuitemadd" method="post" id="Paramform"
									class="form-horizontal" commandName="menuitem">
									<div class=" col-xs-12 table-header">菜单信息添加<a href="menuitems" style="float:right;color:#fff;">返回权限列表</a></div>
                                    <table id="dynamic-table">
								      ...
								      <tr>
								        ...
								        <c:forEach items="${menuitems}" var="menuitem" varStatus="status">
								          <option value="${menuitem.id }">|_${menuitem.menuName }</option>
							                    ...
							                    <option value="${childrenmenu1.id }">|__${childrenmenu1.menuName }</option>
								                  <menu:children parentId="${childrenmenu1.id }">
										             <c:forEach items="${childrenmenus}" var="childrenmenu2" varStatus="status">
									                    <option value="${childrenmenu2.id }">|___${childrenmenu2.menuName }</option>
									                  </c:forEach>
										          </menu:children>
							                  </c:forEach>
								          </menu:children>
