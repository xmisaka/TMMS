...
$(document).ready(function(){
	...
	$.post(url,function(responseText){
		 ...
		 if(data.length==0){
			 ...
			 for(var i=0;i<data.length;i++){
				 if(data[i]["collegeId"]==collegeId){
					 findSpecialty(data[i]["collegeId"]);
					 content+="<option value=\""+data[i]["collegeId"]+"\" selected>"+data[i]["collegeName"]+"</option>"
				 }else{
...
function findSpecialty(collegeId){
	...
	$.post(url,params,function(responseText){
		 ...
		 if(data.length==0){
			 ...
			 for(var i=0;i<data.length;i++){
				 if(data[i]["specialtyId"]==specialtyId){
					 findClass(data[i]["specialtyId"]);
					 content+="<option value=\""+data[i]["specialtyId"]+"\" selected>"+data[i]["specialtyName"]+"</option>"
				 }else{
					 content+="<option value=\""+data[i]["specialtyId"]+"\">"+data[i]["specialtyName"]+"</option>"
				 }
			 }
	...
	$.post(url,params,function(responseText){
		 ...
		 for(var i=0;i<data.length;i++){
			 if(data[i]["classId"]==classId){
				 content+="<option value=\""+data[i]["classId"]+"\" selected>"+data[i]["className"]+"</option>"
			 }else{
				 content+="<option value=\""+data[i]["classId"]+"\">"+data[i]["className"]+"</option>"
			 }
		 }
